# Documentation Index

This directory contains all project documentation organized by category.

## Structure

- **[architecture/](architecture/)** - System architecture and design documentation
- **[verification/](verification/)** - Testing and verification strategies
- **[build-notes/](build-notes/)** - Build instructions, BOM, and development notes
- **[media/](media/)** - Images, screenshots, schematics, and other media files

## Quick Links

- [Architecture Overview](architecture/overview.md)
- [Verification Procedures](verification/procedures.md)
- [Opcode Map](../spec/opcode/opcode_table.md)
- [Bill of Materials](build-notes/bom.md)
